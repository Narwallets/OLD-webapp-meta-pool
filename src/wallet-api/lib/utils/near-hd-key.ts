...
import type * as createHmacNamespace from "../../../../bundled-types/create-hmac.js"
declare var createHmacPackage: typeof createHmacNamespace
...
export function getMasterKeyFromSeed(seed: string): Keys {
    const hmac = createHmacPackage.createHmac('sha512', ED25519_CURVE);
...
const CKDPriv = ({ key, chainCode }, index) => {
    ...
    const I = createHmacPackage.createHmac('sha512', chainCode)
        .update(data)
        .digest();
    const IL = I.slice(0, 32);
    const IR = I.slice(32);
